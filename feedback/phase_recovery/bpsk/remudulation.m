...
%% Модель фапч для приема bpsk сигнала. Фазовый детектор сделан на основе ремодуляции (data-decision).
%% Принимается решение о том, какой символ был передан. Далее фаза входного отсчет сравнивается
%% с фазой принятого символа. Таким образом вычисдяется ошибка по фазе.  

%% формула для pill-in time дает меньшее значение.
...
Tp = (freq_offset)^2 * (1/pi^2) / (ksi*wn^3);          % pull-in time 
