...
%% Модель фапч для приема bpsk сигнала. Фазовый детектор сделан на основе функции sign().
%% Для bpsk синхронизация достигается в случае, если мнимая часть равна нулю. С помощью
%% функции sign() для действительной части определяется направление вращения.
%% Такой детектор эквивалентен детектору A*sin(delta_pi)
...
Tp = (freq_offset)^2 / (2*ksi*wn^3);          % pull-in time 
...
for n = 2:Nsamp
    ...
    %% фазовый детектор
    err(n) = Kd * sign(real(detector_input(n))) * imag(detector_input(n));
