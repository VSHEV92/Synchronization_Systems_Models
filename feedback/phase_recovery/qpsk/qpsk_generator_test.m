...
[tx_bits, tx_samples] = qpsk_generator(...
...
for SNR = SNR_list
    ...
    % демодуляция данных
    rx_symbols = pskdemod(tx_samples, 4, pi/4);
    rx_bits = int2bit(rx_symbols, 2);
    rx_bits = reshape(rx_bits, 1, 2 * symbols_number);

    % сравнение данных и подсчет ошибок
    number_of_errors = sum(tx_bits ~= rx_bits);
    P_err = [P_err number_of_errors/(2 * symbols_number)];
end
figure(2)
